...
Available on the [Mac App Store](https://apps.apple.com/in/app/keyholdr/id6789253781?mt=12).

### Added
- **New app icon** — the website's key-and-balloon logo on a paper tile. Brand
  assets (icon, one-colour key mark, menu bar glyph) are generated by
  `branding/generate.py`; the menu bar still uses the `key.fill` symbol.
- `branding/appstore/render.sh` renders the App Store screenshots at every
  required Mac size.
- `CLAUDE.md` with build, test and architecture notes for contributors.

### Fixed
- **⌃⌥⌘K did nothing on macOS 27** — the popover library (MenuBarExtraAccess)
  couldn't open the menu bar popover from code there. Updated to 1.3.1. The
  same fix restores the popover reopening after a Touch ID prompt.
- **The first ⌃⌥⌘K after launch was wasted** — the popover was marked open
  before it ever was, so the first press only "closed" it.
- **⌘N always did the same thing** — it opened a new key or a new note depending
  on which tab was saved when the app launched, ignoring the tab you were on.
  It now follows the current tab, and does nothing while the export/import form
  or "How Keyholdr Works…" is showing.
- **Terminal UI:** a note saved with ⌃S and then emptied is now deleted when you
  leave the editor, instead of keeping its old text.

### Changed
- New App Store screenshots, with a neutral example (Twilio) in place of OpenAI.
- README, CONTRIBUTING and `docs/CLI.md` now state that building needs full
  Xcode (the Command Line Tools alone can't build on the current SDK), and
  document `⌃S` in the terminal UI's add form and note editor.
...
[Unreleased]: https://github.com/OlixIgnacious/keyholdr/compare/v1.7.1...HEAD
[1.7.1]: https://github.com/OlixIgnacious/keyholdr/compare/v1.7.0...v1.7.1
